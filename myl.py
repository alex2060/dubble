...
def sol_f_1_l_F(number_of_floors,egonval):
	...
	for x in range(number_of_floors):
		...
		if egonval[0][x]>=0:
			add=con2[x]*sinh(math.sqrt(abs(egonval[0][x]))*(time+con1[x]))
		else:
			add=con2[x]*sin(math.sqrt(abs(egonval[0][x]))*(time+con1[x]))
		for y in range(number_of_floors):
...
def sol_f_1rows_l_F(number_of_floors,egonval):
	...
	for y in range(number_of_floors):
		...
		if egonval[0][y]>=0:
			add=cont2[y]*sinh(math.sqrt(abs(egonval[0][y]))*(time+cont1[y]))
		else:
			add=cont2[y]*sin(math.sqrt(abs(egonval[0][y]))*(time+cont1[y]))
